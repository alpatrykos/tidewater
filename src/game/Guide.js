...
const CARDS = [
	...
	},
	{
		...
		body: `<div class="gm-guide-list">
			...
			${ row( k( 'B' ), 'Take out a fresh Żubr, open it, finish the drink and toss the empty can — all with one press. There is always another ready' ) }
...
export class Guide {
	...
	show( i ) {
		...
		this.body.innerHTML = this.game.app.input.touchMode ? c.body.replace( '<kbd>B</kbd>', '<kbd>Drink Żubr</kbd>' ).replace( 'all with one press', 'all with one tap' ) : c.body;
